...
const POSTER_TITLES = [
    ...
    "WEDNESDAY", "THE\nWITCHER", "OUTER\nBANKS", "SPIDER-MAN\nNO WAY HOME",
    "THE NIGHT\nAGENT", "MONEY\nHEIST", "STRANGER\nTHINGS", "GLADIATOR",
    "EMILY IN\nPARIS", "THE MOTHER", "COBRA KAI", "SQUID GAME",
    "DARK", "OZARK", "BRIDGERTON", "LUPIN", "YOU", "NARCOS",
];

const COL_COUNT = 4;
const ITEMS_PER_COL = 6;

const COLUMNS: string[][] = Array.from({ length: COL_COUNT }, (_, i) =>
    Array.from({ length: ITEMS_PER_COL * 2 }, (_, j) =>
        POSTER_TITLES[(i * ITEMS_PER_COL + j) % POSTER_TITLES.length]
    )
);
                ...
                <div className="poster-grid">
                    {COLUMNS.map((col, ci) => (
                        <div key={ci} className="poster-col">
                            {col.map((title, ti) => (
                                <div key={`${ci}-${ti}`} className={`poster-item pc-${((ci * ITEMS_PER_COL + ti) % 16) + 1}`}>
                                    <span className="poster-badge">N</span>
